...
- ```number```: A symbolic number used to differentiate between outputs of multiple runs
- ```cooldownTime```: Reserved time after granted priority when no priority can be granted
- ```red_min_duration_coefficient```: Minimum passed phased duration before priority can be granted
- ```way```: Compensation or priority strategy, following 5 options are available
...
- ```mode```: either ```fixed``` or ```actuated``` 
- ```simulationTime```: Time until which to run the simulation (full simulation ends at 39600)
...
Download the repository onto your device. To successfully run it make sure your ```$SUMO_HOME``` is set up correctly. 
